...
## Operational Notes
- **Latency instrumentation** – The HUD's **E2E** row shows true per-frame capture→render latency (p50/p95 per eye): the server streams per-frame capture timestamps over the data channel, the client clock-syncs NTP-style and matches frames via `requestVideoFrameCallback` RTP timestamps. Use the HUD **Rec 60s** button to save a full metrics report to `reports/` on the server and `scripts/compare_reports.py` to diff two runs; certify the metric once per setup with the physical blink test (`web/blink.html`, procedure in `docs/pi_streaming_checklist.md`).
